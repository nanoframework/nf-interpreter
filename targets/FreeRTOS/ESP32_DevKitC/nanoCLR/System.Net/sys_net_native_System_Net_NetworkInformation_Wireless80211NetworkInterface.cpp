...
HRESULT Library_sys_net_native_System_Net_NetworkInformation_Wireless80211NetworkInterface::UpdateConfiguration___STATIC__VOID__SystemNetNetworkInformationWireless80211__BOOLEAN( CLR_RT_StackFrame& stack )
{
    ...
    if (pNetworkKey->m_numOfElements > sizeof(config.networkKey)) NANOCLR_SET_AND_LEAVE(CLR_E_INVALID_PARAMETER);        
    config.networkKeyLength = pNetworkKey->m_numOfElements;

    if (pReKeyInternal->m_numOfElements > sizeof(config.reKeyInternal)) NANOCLR_SET_AND_LEAVE(CLR_E_INVALID_PARAMETER);
