    ...
    *errorCode = AccessMemoryErrorCode_NoError;
    ...
    switch(mode)
    {
        case AccessMemory_Write:
            ...
            return Esp32FlashDriver_Write((void*)0, location, lengthInBytes, buffer, false);

        case AccessMemory_Erase:
            // erase using FLASH driver
            return  Esp32FlashDriver_EraseBlock((void*)0, location);


        case AccessMemory_Check:
            // use FLASH driver to check is FLASH segment is erased
            return  Esp32FlashDriver_IsBlockErased((void*)0, location, lengthInBytes);
