...
{
    // 'fake' function pointer to load nanoCLR ResetHandler address
    irq_vector_t JumpToNanoCLR;

    // load address of init stack
    const uint32_t* initStack = (uint32_t*) address;

    // load nanoCLR vector table
    const uint32_t* nanoCLRVectorTable = (uint32_t*)address;

    // load the jump address with the nanoCLR ResetHandler address
    JumpToNanoCLR = (irq_vector_t)++nanoCLRVectorTable;

    // disable all interrupts in FreeRTOS
    ...
    __set_MSP((uint32_t)initStack);

    // make the jump to nanoCLR, calling the 'fake' function
    JumpToNanoCLR();
}
...
bool CheckValidCLRImage(uint32_t address)
{
    ...
    // load nanoCLR vector table
    uint32_t* nanoCLRVectorTable = (uint32_t*)address;
    ...
    resetVectorAddress = (uint32_t)*(uint32_t**)((uint32_t*)++nanoCLRVectorTable);
    ...
    uint32_t opCodeAddress = (uint32_t)((uint32_t**)nanoCLRVectorTable);
