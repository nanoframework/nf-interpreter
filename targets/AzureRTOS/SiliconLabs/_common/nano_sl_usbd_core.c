...
// This is an extension to the original file from Gecko SDK v4.3.0
// Following
// https://community.silabs.com/s/question/0D58Y0000AGPoMDSQ1/how-to-add-a-new-usb-interface-when-the-device-is-already-started
// We have to hack our way into this. Stopping the USB core is not a decent solution because it momentarily interrupts
// the USB connection, thus causing a debug session to be lost.
// The workaround is to skip the check for the USB Core being started.
// It was found that, despite its started state, it's OK to add an interface to the USB Core.
