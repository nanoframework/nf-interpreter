...
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::GetAddrInfo(nodename, servname, hints, res);
}
void HAL_SOCK_freeaddrinfo(struct SOCK_addrinfo *ai)
{
    NATIVE_PROFILE_PAL_NETWORK();
    ISM43362_SOCKETS_Driver::FreeAddrInfo(ai);
}
int HAL_SOCK_ioctl(SOCK_SOCKET socket, int cmd, int *data)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::Ioctl(socket, cmd, data);
}
int HAL_SOCK_getlasterror()
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::GetLastError();
}
int HAL_SOCK_getsocklasterror(SOCK_SOCKET socket)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::GetSockLastError(socket);
}
int HAL_SOCK_select(
    int nfds,
    SOCK_fd_set *readfds,
...
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::UpdateAdapterConfiguration(interfaceIndex, updateFlags, config);
}

HRESULT HAL_SOCK_CONFIGURATION_Link_status(uint32_t interfaceIndex, bool *status)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::Link_status(interfaceIndex, status);
}

HRESULT HAL_SOCK_IPAddressFromString(const char *ipString, uint64_t *address)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::IPAddressFromString(ipString, address);
}

const char *HAL_SOCK_IPAddressToString(uint32_t address)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return ISM43362_SOCKETS_Driver::IPAddressToString(address);
}
