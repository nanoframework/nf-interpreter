...
Sockets_ISM43362_Driver g_Sockets_ISM43362_Driver;
...
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Connect(socket, address, addressLen);
}
int SOCK_send(SOCK_SOCKET socket, const char *buf, int len, int flags)
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Send(socket, buf, len, flags);
}
int SOCK_recv(SOCK_SOCKET socket, char *buf, int len, int flags)
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Recv(socket, buf, len, flags);
}
int SOCK_close(SOCK_SOCKET socket)
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Close(socket);
}
int SOCK_listen(SOCK_SOCKET socket, int backlog)
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Listen(socket, backlog);
}
...
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Select(nfds, readfds, writefds, except, timeout);
}
...
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::RecvFrom(s, buf, len, flags, from, fromlen);
}
...
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_ISM43362_Driver::Initialize();
}
...
bool Network_Uninitialize()
{
    ...
    return Sockets_ISM43362_Driver::Uninitialize();
}
...
//
//-----------------------------------------------------------------------------
void Sockets_ISM43362_Driver::CloseConnections(bool fCloseDbg)
{
    NATIVE_PROFILE_PAL_COM();

    int32_t cnt = g_Sockets_ISM43362_Driver.m_cntSockets;
    int32_t idx = 0;

    // round one - close all SSL sockets
    for (int32_t i = cnt - 1; i >= 0; i--)
    {
        struct SocketRegisterMap &entry = g_Sockets_ISM43362_Driver.m_socketHandles[i];
    ...
    cnt = g_Sockets_ISM43362_Driver.m_cntSockets;
    ...
    for (int32_t i = 0; i < cnt; i++)
    {
        ...
        if (entry.m_socket != SOCK_SOCKET_ERROR)
        {
            if (fCloseDbg || !ISSET_SOCKET_FLAG(entry, SocketRegisterMap::c_DebugSocket))
            {
                ...
                g_Sockets_ISM43362_Driver.m_socketHandles[i].m_socket = SOCK_SOCKET_ERROR;
                g_Sockets_ISM43362_Driver.m_socketHandles[i].m_flags = 0;
                g_Sockets_ISM43362_Driver.m_socketHandles[i].m_sslData = NULL;
                g_Sockets_ISM43362_Driver.m_cntSockets--;
            }
            else if (i > 0)
            {
                memcpy(
                    &g_Sockets_ISM43362_Driver.m_socketHandles[i],
                    &g_Sockets_ISM43362_Driver.m_socketHandles[idx++],
                    sizeof(g_Sockets_ISM43362_Driver.m_socketHandles[i]));
            }
        }
    }
}
...
SOCK_SOCKET Sockets_ISM43362_Driver::Socket(int32_t family, int32_t type, int32_t protocol, bool fDebug)
{
    NATIVE_PROFILE_PAL_COM();
...
int Sockets_ISM43362_Driver::Connect(SOCK_SOCKET socket, const struct SOCK_sockaddr *address, int32_t addressLen)
{
    NATIVE_PROFILE_PAL_COM();

    return HAL_SOCK_connect(socket, address, addressLen);
}
int Sockets_ISM43362_Driver::Send(SOCK_SOCKET socket, const char *buf, int32_t len, int32_t flags)
{
    NATIVE_PROFILE_PAL_COM();

    return HAL_SOCK_send(socket, buf, len, flags);
}
int Sockets_ISM43362_Driver::Recv(SOCK_SOCKET socket, char *buf, int32_t len, int32_t flags)
{
    NATIVE_PROFILE_PAL_COM();
...
int Sockets_ISM43362_Driver::Shutdown(SOCK_SOCKET sock, int32_t how)
{
    NATIVE_PROFILE_PAL_COM();
...
//
//-----------------------------------------------------------------------------
int Sockets_ISM43362_Driver::Close(SOCK_SOCKET sock)
{
    NATIVE_PROFILE_PAL_COM();
    UnregisterSocket(sock);
    return HAL_SOCK_close(sock);
}
int Sockets_ISM43362_Driver::Listen(SOCK_SOCKET socket, int32_t backlog)
{
    NATIVE_PROFILE_PAL_COM();
...
int Sockets_ISM43362_Driver::Select(
    int32_t nfds,
    SOCK_fd_set *readfds,
    SOCK_fd_set *writefds,
...
int Sockets_ISM43362_Driver::RecvFrom(
    SOCK_SOCKET s,
    char *buf,
    int32_t len,
    int32_t flags,
...
int Sockets_ISM43362_Driver::SendTo(
    SOCK_SOCKET s,
    const char *buf,
    int32_t len,
    int32_t flags,
...
bool Sockets_ISM43362_Driver::Initialize()
{
    NATIVE_PROFILE_PAL_COM();
    SOCKET_CHECK_ENTER();
...
__nfweak void Sockets_ISM43362_Driver::Debugger_Abort()
{
}
__nfweak void Sockets_ISM43362_Driver::Debugger_Uninitialize()
{
}
...
//
//-----------------------------------------------------------------------------
void Sockets_ISM43362_Driver::RegisterSocket(SOCK_SOCKET sock, bool selectable, bool fDebug)
{
    (void)selectable;
    ...
    if (g_Sockets_ISM43362_Driver.m_cntSockets >= SOCKETS_MAX_COUNT)
        return;

    GLOBAL_LOCK_SOCKETS(lock);

    g_Sockets_ISM43362_Driver.m_socketHandles[g_Sockets_ISM43362_Driver.m_cntSockets].m_socket = sock;
    g_Sockets_ISM43362_Driver.m_socketHandles[g_Sockets_ISM43362_Driver.m_cntSockets].m_flags = 0;
    g_Sockets_ISM43362_Driver.m_socketHandles[g_Sockets_ISM43362_Driver.m_cntSockets].m_sslData = NULL;

    if (fDebug)
        SET_SOCKET_FLAG(
            g_Sockets_ISM43362_Driver.m_socketHandles[g_Sockets_ISM43362_Driver.m_cntSockets],
...
//
//-----------------------------------------------------------------------------
void Sockets_ISM43362_Driver::UnregisterSocket(SOCK_SOCKET sock)
{
    int32_t index = -1;
    ...
    g_Sockets_ISM43362_Driver.GetSocketSslData(sock, index);
    ...
    if (index != g_Sockets_ISM43362_Driver.m_cntSockets)
    {
        memcpy(
            &g_Sockets_ISM43362_Driver.m_socketHandles[index],
            &g_Sockets_ISM43362_Driver.m_socketHandles[g_Sockets_ISM43362_Driver.m_cntSockets],
            sizeof(g_Sockets_ISM43362_Driver.m_socketHandles[index]));
    }
