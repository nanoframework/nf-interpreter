...
#define GattDeviceService Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattDeviceService
#define GattCharacteristic Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattCharacteristic
#define GattDescriptor Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattDescriptor

enum centralOp { connect, discoverServices, discoverCharacteristics };
...
///
void PostCentralEvent( uint8_t op, uint16_t conn_handle, uint16_t status, uint16_t serviceHandle, uint16_t characteristicHandle)
{
...
//
bool PostAndWaitCentralEvent(uint8_t op, uint16_t conn_handle, uint16_t status, uint16_t serviceHandle, uint16_t characteristicHandle)
{
    EventBits_t uxBits;
...
int central_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type)
    {
       case BLE_GAP_EVENT_CONNECT:
            ...
            BLE_DEBUG_PRINTF("BLE_GAP_EVENT_CONNECT route:%X status:%X handle:%X\n", con->connectionHandle, event->connect.status, conn_handle);

            // Complete connect using temporary connection  handle saved in context, passing in real connection Handle on Characteristic handle param 
            PostCentralEvent(BluetoothEventType_ConnectComplete, con->connectionHandle, event->connect.status, 0 , conn_handle);
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ...
            PostCentralEvent(BluetoothEventType_ConnectionDisconnected, conn_handle, event->connect.status, 0 , conn_handle);
            break;

        case BLE_GAP_EVENT_NOTIFY_RX:
            {
                // Device sent us a notification or indication.
                BLE_DEBUG_PRINTF("Received %s; conn_handle=%d attr_handle=%d "
                                "attr_len=%d\n",
                ...
                PostAndWaitCentralEvent(BluetoothEventType_AttributeValueChanged, event->notify_rx.conn_handle, event->notify_rx.indication, 0 , event->notify_rx.attr_handle);
            }
            break;
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattCharacteristic::NativeReadEventData___SZARRAY_U1__U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeConnect___U2__U8__nanoFrameworkDeviceBluetoothBluetoothAddressType__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        int rc;
        ...
        if (pThis[Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::FIELD___disposed].NumericByRef().u1 != 0)
        {
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeDisconnect___VOID__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeDispose___VOID__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        ushort conn_Handle = stack.Arg1().NumericByRef().u2;        
...
//
int DiscoverServicesCb(uint16_t conn_handle, const struct ble_gatt_error *error, const struct ble_gatt_svc *service, void *arg)
{
    central_context *con;
    ...
    if (error->status == 0)
    {
        ...
        if (!PostAndWaitCentralEvent(BluetoothEventType_ServiceDiscovered , conn_handle, error->status, ptr->start_handle, 0) )
        {
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeDiscoverServices___U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        ushort conn_Handle = stack.Arg1().NumericByRef().u2;
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeUpdateService___VOID__U2__nanoFrameworkDeviceBluetoothGenericAttributeProfileGattDeviceService( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        central_context * con;
...
//
static int DiscoverChrCb(uint16_t conn_handle, 
                  ...
                  const struct ble_gatt_chr *chr, void *arg)
{
    ...
    if (error->status == 0)
    {
        ...
        BLE_DEBUG_PRINTF("DiscoverChrCb call post CH:%X def handle %d val handle %d\n", conn_handle, chr->def_handle, chr->val_handle);

        if (!PostAndWaitCentralEvent(BluetoothEventType_CharacteristicDiscovered, conn_handle, error->status, con->serviceHandle, chr->val_handle))
        {
    ...
    else
    {
        ...
        // Complete / error 
        PostCentralEvent(BluetoothEventType_CharacteristicDiscoveryComplete, conn_handle, error->status, con->serviceHandle, 0);
    }

    return 0;
}

HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattDeviceService::NativeDiscoverCharacteristics___U2__U2__U2__U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        int rc;
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattDeviceService::NativeUpdateCharacteristic___VOID__U2__U2__U2__nanoFrameworkDeviceBluetoothGenericAttributeProfileGattCharacteristic( CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
    {
        ...
        pGattCharacteristic[GattCharacteristic::FIELD___characteristicProperties].NumericByRef().u4 = (int)characteristic->properties;
...
static int ReadCharCallback(uint16_t conn_handle,
                const struct ble_gatt_error *error,
                ...
                void *arg)
{
    ...
    if (error->status==0)
    {
         BLE_DEBUG_PRINTF("Read complete; status=%d conn_handle=%d handle:%d\n", error->status, conn_handle, attr->handle);
    }
    else
    {
    ...
    if (error->status == 0)
    {
        ...
        if (PostAndWaitCentralEvent(BluetoothEventType_AttributeReadValueComplete, conn_handle, error->status, con->serviceHandle, attr->handle))
        {
            //TODO ?
    ...
    {
        PostCentralEvent(BluetoothEventType_AttributeReadValueComplete, conn_handle, error->status, con->serviceHandle, 0);
    }

    return 0;
}

HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeStartReadValue___U2__U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        int rc;
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeReadValue___SZARRAY_U1__U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        uint16_t conn_handle = stack.Arg1().NumericByRef().u2;
...
static int writeCharWithResponseCallback(uint16_t conn_handle,
                 const struct ble_gatt_error *error,
                 struct ble_gatt_attr *attr,
                 void *arg)
{
    ...
    BLE_DEBUG_PRINTF("Write complete; status=%d conn_handle=%d attr_handle=%d\n", error->status, conn_handle, attr->handle);

    PostCentralEvent(BluetoothEventType_AttributeWriteValueComplete, conn_handle, error->status, con->serviceHandle, attr->handle);

    return 0;
}

HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_BluetoothLEDevice::NativeStartWriteValue___U2__U2__U2__BOOLEAN__SZARRAY_U1__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        ...
        {
            BLE_DEBUG_PRINTF("NativeStartWriteValue con:%d val handle:%d data len:%d\n", conn_handle, val_handle, bufferLen);
            ...
            if (withResponse)
            {
                ...
                rc = ble_gattc_write_flat(conn_handle, val_handle, bufferPtr, bufferLen, writeCharWithResponseCallback, (void*)con);
            }
            else
            {
...
static int descriptorDiscoveryCallback(uint16_t conn_handle, const struct ble_gatt_error *error,
                                 ...
                                 void *arg)
{
    ...
    if (error->status == 0)
    {
        ...
        if (!PostAndWaitCentralEvent(BluetoothEventType_DescriptorDiscovered, conn_handle, error->status, con->serviceHandle, chr_val_handle))
        {
    ...
    else
    {
        ...
        // Complete / error 
        PostCentralEvent(BluetoothEventType_DescriptorDiscoveryComplete, conn_handle, error->status, con->serviceHandle, chr_val_handle);
    }

    return 0;
}

HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattCharacteristic::NativeStartDiscoveryDescriptors___U2__U2__U2__U2( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
        ushort conn_handle = stack.Arg1().NumericByRef().u2;
        ...
        int rc = ble_gattc_disc_all_dscs(conn_handle, start_handle, end_handle, descriptorDiscoveryCallback, (void*)con);

        BLE_DEBUG_PRINTF("NativeStartDiscoveryDescriptors con:%d start:%d end:%d rc:%d\n", conn_handle,start_handle, end_handle, rc);
...
HRESULT Library_sys_dev_ble_native_nanoFramework_Device_Bluetooth_GenericAttributeProfile_GattCharacteristic::NativeUpdateDescriptor___VOID__U2__U2__U2__nanoFrameworkDeviceBluetoothGenericAttributeProfileGattDescriptor( CLR_RT_StackFrame &stack )
{
    NANOCLR_HEADER();
    {
