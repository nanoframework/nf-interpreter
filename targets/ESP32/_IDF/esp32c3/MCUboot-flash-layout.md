# MCUboot Flash Layout — esp32c3 (RISC-V RV32IMC, WiFi + BT LE)
...
Same structure as 8 MB; deploy size extends to ~12.6 MB (0xC92000).

## Notes

- The existing non-MCUboot 2MB CSV uses `spiffs` config subtype. MCUboot variants use `littlefs` throughout.
