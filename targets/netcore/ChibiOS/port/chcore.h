 ...
 */
#define PORT_CORE_VARIANT_NAME "x64 (integer only)"

/**
 * @brief   Name of the compiler supported by this port.
 */
#define PORT_COMPILER_NAME "MSVC " __VERSION__
...
#define PORT_IDLE_THREAD_STACK_SIZE 16384
#endif
...
/**
 * @brief   Type of a generic x64 register.
 */
typedef void *regx64;
...
#define AALIGN(p, mask, mod) p = (void *)((((uint64_t)(p) - (uint64_t)(mod)) & ~(uint64_t)(mask)) + (uint64_t)(mod))
...
#define PORT_WA_SIZE(n)                                                                                                \
    ((sizeof(void *) * 4U) + sizeof(struct port_intctx) + ((size_t)(n)) + ((size_t)(PORT_INT_REQUIRED_STACK)) +        \
     (sizeof(void *) - 1)) &                                                                                           \
        ~(sizeof(void *) - 1)
 ...
 */
#define PORT_WORKING_AREA(s, n) stkalign_t s[(THD_WORKING_AREA_SIZE(n) + sizeof(stkalign_t) - 1) / sizeof(stkalign_t)]
...
extern "C"
{
    ...
    __declspec(noreturn) void _port_thread_start(msg_t (*pf)(void *p), void *p);
    /*lint -restore*/
