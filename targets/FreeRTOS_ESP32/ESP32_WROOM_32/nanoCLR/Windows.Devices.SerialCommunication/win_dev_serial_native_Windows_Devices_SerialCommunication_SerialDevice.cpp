...
// in UWP the COM ports are named COM1, COM2, COM3. But ESP32 uses internally
// UART0, UART1, UART2. This maps the port index 1, 2 or 3 to the uart number 0,
// 1 or 2
#define PORT_INDEX_TO_UART_NUM(portIndex) ((portIndex)-1)
// in UWP the COM ports are named COM1, COM2, COM3. But ESP32 uses internally
// UART0, UART1, UART2. This maps the uart number 0, 1 or 2 to the port index 1,
// 2 or 3
...
void UnitializePalUart(NF_PAL_UART *palUart) {
  if (palUart && palUart->SerialDevice) {
    ...
    Esp32_SetMappedDevicePins(DEV_TYPE_SERIAL, palUart->UartNum,
                              Esp32SerialPin_Tx, UART_PIN_NO_CHANGE);
    Esp32_SetMappedDevicePins(DEV_TYPE_SERIAL, palUart->UartNum,
                              Esp32SerialPin_Rx, UART_PIN_NO_CHANGE);
    Esp32_SetMappedDevicePins(DEV_TYPE_SERIAL, palUart->UartNum,
                              Esp32SerialPin_Rts, UART_PIN_NO_CHANGE);
    Esp32_SetMappedDevicePins(DEV_TYPE_SERIAL, palUart->UartNum,
                              Esp32SerialPin_Cts, UART_PIN_NO_CHANGE);
  }
}
...
void uart_event_task(void *pvParameters) {
  ...
  while (run) {
    ...
    if (xQueueReceive(palUart->UartEventQueue, &event,
      ...
      if (readData) {
        ...
        if (bufferedSize > 0) {
          ...
          if (buffer) {
            // try to read RX FIFO
            readCount =
                uart_read_bytes(palUart->UartNum, buffer, bufferedSize, 1);
            ...
            if (palUart->RxBytesToRead > 0) {
              ...
              // post a managed event with the port index and event code (check
              // if there is a watch char in the buffer or just any char)
              //  check if callbacks are registered so this is called only if
              //  there is anyone listening otherwise don't bother
              if (palUart
                      ->SerialDevice
                          [Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
                               FIELD___callbacksDataReceivedEvent]
                      .Dereference() != NULL) {
                PostManagedEvent(EVENT_SERIAL, 0,
                                 UART_NUM_TO_PORT_INDEX(palUart->UartNum),
                                 (watchCharPos > -1) ? SerialData_WatchChar
                                                     : SerialData_Chars);
              }
            }
          }
...
void UartTxWorkerTask(void *pvParameters) {
  ...
  uart_write_bytes(palUart->UartNum,
                   (const char *)palUart->TxRingBuffer.Reader(),
...
bool IsLongRunningOperation(uint32_t bufferSize, uint32_t baudRate,
...
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
  ...
  uart_num = (uart_port_t)PORT_INDEX_TO_UART_NUM(
      pThis[FIELD___portIndex].NumericByRef().s4);
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    NativeInit___VOID(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
  ...
  if (xTaskCreatePinnedToCore(uart_event_task, task_name, 2048, palUart, 12,
                              &(palUart->UartEventTask), 1) != pdPASS) {
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    NativeConfig___VOID(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
  {
    ...
    // Get Uart number for serial device
    uart_port_t uart_num = (uart_port_t)PORT_INDEX_TO_UART_NUM(
        pThis[FIELD___portIndex].NumericByRef().s4);
    if (uart_num > UART_NUM_2 || uart_num < 0) {
      NANOCLR_SET_AND_LEAVE(CLR_E_INVALID_PARAMETER);
    }
    ...
    if (GetPalUartFromUartNum(uart_num)->SerialDevice) {
      ...
      errors += uart_set_hw_flow_ctrl(uart_num, uart_config.flow_ctrl,
      ...
      // If you call this once driver installed it resets UART and stop events
      // ISR being called.
    ...
    int txPin =
        Esp32_GetMappedDevicePins(DEV_TYPE_SERIAL, uart_num, Esp32SerialPin_Tx);
    int rxPin =
        Esp32_GetMappedDevicePins(DEV_TYPE_SERIAL, uart_num, Esp32SerialPin_Rx);
    int rtsPin = Esp32_GetMappedDevicePins(DEV_TYPE_SERIAL, uart_num,
                                           Esp32SerialPin_Rts);
    int ctsPin = Esp32_GetMappedDevicePins(DEV_TYPE_SERIAL, uart_num,
                                           Esp32SerialPin_Cts);
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    NativeWrite___VOID__SZARRAY_U1(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
  {
    NF_PAL_UART *palUart = NULL;
    ...
    if (palUart->TxRingBuffer.Capacity() - palUart->TxRingBuffer.Length() <
        length) {
    ...
    // need to update the _unstoredBufferLength field in the
    // SerialDeviceOutputStream get pointer to outputStream field
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    NativeStore___U4(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
  ...
  if (palUart->TxRingBuffer.Length() > 0) {
    ...
    if (palUart->TxOngoingCount == 0) {
      ...
      // need to wait for the ongoing operation to complete before starting a
      // new one
    }
  }
  ...
  if (txOk) {
    ...
    if (isLongRunningOperation) {
      // setup timeout
      NANOCLR_CHECK_HRESULT(stack.SetupTimeoutFromTimeSpan(
          pThis[FIELD___writeTimeout], timeoutTicks));
      ...
      if (stack.m_customState == 1) {
        ...
        if (xTaskCreate(UartTxWorkerTask, task_name, 2048, palUart, 12, NULL) !=
            pdPASS) {
      ...
      // by design: don't bother checking the return value
      uart_write_bytes(uart_num, (const char *)palUart->TxRingBuffer.Reader(),
                       length);
  ...
  while (eventResult) {
    ...
    // non-blocking wait allowing other threads to run while we wait for the Tx
    // operation to complete
    NANOCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.WaitEvents(
        stack.m_owningThread, *timeoutTicks,
        CLR_RT_ExecutionEngine::c_Event_SerialPortOut, eventResult));
...
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
  ...
  NANOCLR_CHECK_HRESULT(
      stack.SetupTimeoutFromTimeSpan(pThis[FIELD___readTimeout], timeoutTicks));
  ...
  while (eventResult) {
    if (stack.m_customState == 1) {
      ...
      NANOCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.WaitEvents(
          stack.m_owningThread, *timeoutTicks,
          CLR_RT_ExecutionEngine::c_Event_SerialPortIn, eventResult));
      ...
      if (!eventResult) {
        ...
        if (options == InputStreamOptions_ReadAhead) {
          ...
          // take InputStreamOptions_Partial as default and read requested
          // quantity or what's available
          bytesToRead = count;
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    NativeSetWatchChar___VOID(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
...
HRESULT
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
    get_BytesToRead___U4(CLR_RT_StackFrame &stack) {
  NANOCLR_HEADER();
...
Library_win_dev_serial_native_Windows_Devices_SerialCommunication_SerialDevice::
  ...
  NANOCLR_HEADER();
  {
    // declare the device selector string whose max size is "COM1,COM2,COM3" +
    // terminator COM1 is being used for VS debug, so it's not available
    char deviceSelectorString[15] = "COM2,COM3";

    // because the caller is expecting a result to be returned
    // we need set a return result in the stack argument using the appropriate
    // SetResult according to the variable type (a string here)
