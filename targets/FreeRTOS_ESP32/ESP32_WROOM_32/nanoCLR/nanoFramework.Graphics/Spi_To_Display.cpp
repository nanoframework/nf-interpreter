...
void SendCommandWord(CLR_UINT16 commandWord)
{
    ASSERT(commandWord != NULL) // unused
    // Not used for this display controller
        return;
}

void SendCommandWordAndData(CLR_UINT16 commandWord, CLR_UINT8 data[], CLR_UINT32 dataCount)
{
    ASSERT(commandWord = commandWord); ASSERT(data[0] = data[0]); ASSERT(dataCount = dataCount);
    // Not used for this display controller
    return;
}
