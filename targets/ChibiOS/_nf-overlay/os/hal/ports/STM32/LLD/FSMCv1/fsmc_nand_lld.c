...
///////////////////////////////////////////////////////////////////////////////
#define NAND_DMA_CHANNEL STM32_DMA_GETCHANNEL(STM32_NAND_DMA_STREAM, STM32_FSMC_DMA_CHN)
...
{
    osalDbgCheck(
        (((uint32_t)ptr % AHB_TRANSACTION_WIDTH) == 0) && ((len % AHB_TRANSACTION_WIDTH) == 0) &&
...
static void nand_ready_isr_enable(NANDDriver *nandp)
{
    ...
    nandp->nand->SR &= ~(FSMC_SR_IRS | FSMC_SR_ILS | FSMC_SR_IFS | FSMC_SR_ILEN | FSMC_SR_IFEN);
...
#error "Incorrect AHB_TRANSACTION_WIDTH"
        ...
        nandp->dmamode = STM32_DMA_CR_CHSEL(NAND_DMA_CHANNEL) | STM32_DMA_CR_PL(STM32_NAND_NAND1_DMA_PRIORITY) |
                         dmasize | STM32_DMA_CR_DMEIE | STM32_DMA_CR_TEIE | STM32_DMA_CR_TCIE;
...
#endif
        nandp->nand->PCR = pcr_bus_width | calc_eccps(nandp) | FSMC_PCR_PTYP_NAND | FSMC_PCR_PBKEN;
...
void nand_lld_read_data(NANDDriver *nandp, uint16_t *data, size_t datalen, uint8_t *addr, size_t addrlen, uint32_t *ecc)
{
    ...
    osalDbgAssert((nandp->nand->PCR & FSMC_PCR_ECCEN) == 0, "State machine broken. ECCEN must be previously disabled.");
    ...
    uint32_t *ecc)
{
    ...
    dmaStartMemCopy(nandp->dma, nandp->dmamode, data, nandp->map_data, datalen / AHB_TRANSACTION_WIDTH);
