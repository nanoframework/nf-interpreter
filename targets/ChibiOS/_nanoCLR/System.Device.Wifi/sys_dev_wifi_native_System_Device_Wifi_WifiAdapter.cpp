...
#if defined(TARGET_HAS_WIFI_ISM43362)
            // ISM43362 never signals Event_Wifi_Station, so waiting on the full connect deadline would only re-check
            // Network_Interface_Connect_Result() once poll every 500ms, without exceeding the real connect deadline
