...
// Updated by HAL_Time_SetUtcTime() and applied on reads.
...
uint64_t  HAL_Time_CurrentDateTime(bool datePartOnly)
{
	...
	chSysLock();
	currentTime =
		(uint64_t)((int64_t)HAL_Time_SysTicksToTime(chVTGetTimeStampI()) + s_utcOffsetTicks);
	chSysUnlock();
...
void HAL_Time_SetUtcTime(uint64_t utcTime)
{
    ...
    // Keep UTC as an offset over monotonic HAL ticks without changing scheduler time.
    chSysLock();
    s_utcOffsetTicks =
        (int64_t)utcTime - (int64_t)HAL_Time_SysTicksToTime(chVTGetTimeStampI());
    chSysUnlock();
