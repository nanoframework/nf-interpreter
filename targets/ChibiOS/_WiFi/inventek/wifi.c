...
WIFI_Status_t WIFI_Init(void)
{
  ...
  ISM43362_DebugPrintf("[ISM43362] WIFI_Init: registering bus IO...\r\n");
  ...
  {
    ISM43362_DebugPrintf("[ISM43362] WIFI_Init: RegisterBusIO OK, calling ES_WIFI_Init()...\r\n");
  ...
  ISM43362_DebugPrintf("[ISM43362] WIFI_Init: result=%d\r\n", (int)ret);
...
WIFI_Status_t WIFI_ListAccessPoints(WIFI_APs_t *APs, uint8_t AP_MaxNbr)
{
  uint8_t APCount;
  WIFI_Status_t ret = WIFI_STATUS_ERROR;
  // static rather than a local (stack) variable - this struct is ~460 bytes and this function
  // is already several native call frames deep (managed WifiAdapter scan -> this -> ES_WIFI_*),
  // so keep it off the stack
  static ES_WIFI_APs_t esWifiAPs;

  ISM43362_DebugPrintf("[ISM43362] WIFI_ListAccessPoints: calling ES_WIFI_ListAccessPoints()...\r\n");

  ES_WIFI_Status_t listStatus = ES_WIFI_ListAccessPoints(&EsWifiObj, &esWifiAPs);

  ISM43362_DebugPrintf(
      "[ISM43362] WIFI_ListAccessPoints: ES_WIFI_ListAccessPoints() returned %d, nbr=%d\r\n",
      (int)listStatus,
      (int)esWifiAPs.nbr);

  if(listStatus == ES_WIFI_STATUS_OK)
  {
    if(esWifiAPs.nbr > 0)
    {
                             ...
                             WIFI_Ecn_t ecn)
{
  ...
  ISM43362_DebugPrintf("[ISM43362] WIFI_Connect: calling ES_WIFI_Connect()...\r\n");

  ES_WIFI_Status_t connectStatus = ES_WIFI_Connect(&EsWifiObj, SSID, Password, (ES_WIFI_SecurityType_t) ecn);

  ISM43362_DebugPrintf("[ISM43362] WIFI_Connect: ES_WIFI_Connect() returned %d\r\n", (int)connectStatus);

  if(connectStatus == ES_WIFI_STATUS_OK)
  {
    ISM43362_DebugPrintf("[ISM43362] WIFI_Connect: calling ES_WIFI_GetNetworkSettings()...\r\n");
...
{
  ES_WIFI_Status_t esStatus = ES_WIFI_ReceiveData(&EsWifiObj, socket, pdata, Reqlen, RcvDatalen, Timeout);

  if (esStatus == ES_WIFI_STATUS_OK)
  {
    return WIFI_STATUS_OK;
  }

  // the remote peer closing the connection (module reports this as an "ERROR" AT response, e.g.
  // to the R0 receive command, once the peer has closed its end) is a NORMAL, expected condition
  // for any protocol that signals end-of-data via connection close (e.g. a simple HTTP server
  // without Content-Length) - callers need to distinguish this from a genuine I/O error so they
  // can report a clean EOF (0 bytes) rather than a hard error
  if (esStatus == ES_WIFI_STATUS_UNEXPECTED_CLOSED_SOCKET)
  {
    *RcvDatalen = 0;
...
/**
  ...
  *         buffered data or blocking. Used to detect stale/dead sockets (e.g. a pooled HTTP
  *         connection whose peer closed while it was sitting idle) before reusing them.
  * @param  socket : socket handle
  * @retval 1 if the socket is still open/selectable, 0 if it is closed/invalid.
  */
uint8_t WIFI_IsSocketOpen(uint8_t socket)
{
  return ES_WIFI_IsSocketOpen(&EsWifiObj, socket);
}
