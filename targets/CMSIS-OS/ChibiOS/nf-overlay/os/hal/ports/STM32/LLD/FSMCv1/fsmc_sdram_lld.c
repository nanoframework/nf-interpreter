...
static void _sdram_init_sequence(const SDRAMConfig *cfgp)
{
    ...
    _sdram_wait_ready();
    SDRAMD.sdram->SDCMR = FMCCM_AUTO_REFRESH | command_target | (cfgp->sdcmr & FMC_SDCMR_NRFS);
    ...
    _sdram_wait_ready();
    SDRAMD.sdram->SDCMR = FMCCM_LOAD_MODE | command_target | (cfgp->sdcmr & FMC_SDCMR_MRD);
...
void fsmcSdramStart(SDRAMDriver *sdramp, const SDRAMConfig *cfgp)
{
    ...
    osalDbgAssert((sdramp->state == SDRAM_STOP) || (sdramp->state == SDRAM_READY), "SDRAM. Invalid state.");
