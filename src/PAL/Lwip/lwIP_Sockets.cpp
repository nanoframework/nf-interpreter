...
void LWIP_SOCKETS_Driver::Status_callback(struct netif *netif)
{
    if (!PostAddressChangedContinuation.IsLinked())
        ...
        // " : "DOWN")); lcd_printf("         IP: %d.%d.%d.%d\n",
        // (netif->ip_addr.addr >> 0) & 0xFF, 	(netif->ip_addr.addr >> 8) & 0xFF,
        // 	(netif->ip_addr.addr >> 16) & 0xFF,
        // 	(netif->ip_addr.addr >> 24) & 0xFF);
        // lcd_printf("         SM: %d.%d.%d.%d\n", (netif->netmask.addr >> 0) &
        // 0xFF, 	(netif->netmask.addr >> 8) & 0xFF, 	(netif->netmask.addr >> 16) &
        // 0xFF, 	(netif->netmask.addr >> 24) & 0xFF); lcd_printf("         GW:
        // %d.%d.%d.%d\n", (netif->gw.addr >> 0) & 0xFF, 	(netif->gw.addr >> 8) &
        // 0xFF, 	(netif->gw.addr >> 16) & 0xFF, 	(netif->gw.addr >> 24) & 0xFF);
    ...
    for (int i = 0; i < g_TargetConfiguration.NetworkInterfaceConfigs->Count; i++)
    {
        ...
        if (networkInterface)
        {
            ...
            if (0 == i)
            {
                ...
                // FIXME               debug_printf("ip address from interface info:
                // %d.%d.%d.%d\r\n", addr[0], addr[1], addr[2], addr[3]);
            }
        }
    }
...
int LWIP_SOCKETS_Driver::Close(SOCK_SOCKET socket)
{
    ...
    // We have to call lwip_close_r() method otherwise the socket doesn't get
    // freed up and we run out of sockets. We could also call the posix
    // closesocket() which should work for all platforms and change all other
    // methods to call the posix version to be consistent.(TODO)
                                                        ...
                                                        HAL_Configuration_NetworkInterface *config)
{
    ...
    if (enableDHCP)
    {
        // developer note: on legacy source there was a hack of trying to renew
        // before release and also setting the release flag in managed call when the
        // intent was to renew only nowadays lwIP seems to be doing what is told, so
        // no need for these hacks anymore also it's NOT possible to renew & release
        // on the same pass, so adding an extra else-if for that just in case it's
        // request from the managed code
