...
SOCK_SOCKET
HAL_SOCK_socket(int family, int type, int protocol)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Socket(family, type, protocol);
}
int
HAL_SOCK_bind(SOCK_SOCKET socket, const struct SOCK_sockaddr* address, int addressLen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Bind(socket, address, addressLen);
}
int
HAL_SOCK_connect(SOCK_SOCKET socket, const struct SOCK_sockaddr* address, int addressLen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Connect(socket, address, addressLen);
}
int
HAL_SOCK_send(SOCK_SOCKET socket, const char* buf, int len, int flags)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Send(socket, buf, len, flags);
}
int
HAL_SOCK_recv(SOCK_SOCKET socket, char* buf, int len, int flags)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Recv(socket, buf, len, flags);
}
int
HAL_SOCK_close(SOCK_SOCKET socket)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Close(socket);
}
int
HAL_SOCK_listen(SOCK_SOCKET socket, int backlog)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Listen(socket, backlog);
}
SOCK_SOCKET
HAL_SOCK_accept(SOCK_SOCKET socket, struct SOCK_sockaddr* address, int* addressLen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Accept(socket, address, addressLen);
}
int
HAL_SOCK_shutdown(SOCK_SOCKET socket, int how)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::Shutdown(socket, how);
}
int
HAL_SOCK_getaddrinfo(
    const char*                 nodename,
    char*                       servname,
...
void
HAL_SOCK_freeaddrinfo(struct SOCK_addrinfo* ai)
{
    NATIVE_PROFILE_PAL_NETWORK();
    LWIP_SOCKETS_Driver::FreeAddrInfo(ai);
}
int
HAL_SOCK_ioctl(SOCK_SOCKET socket, int cmd, int* data)
{
    NATIVE_PROFILE_PAL_NETWORK();
...
int
HAL_SOCK_getsocklasterror(SOCK_SOCKET socket)
{
    NATIVE_PROFILE_PAL_NETWORK();
...
int
HAL_SOCK_setsockopt(SOCK_SOCKET socket, int level, int optname, const char* optval, int optlen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::SetSockOpt(socket, level, optname, optval, optlen);
}
int
HAL_SOCK_getsockopt(SOCK_SOCKET socket, int level, int optname, char* optval, int* optlen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::GetSockOpt(socket, level, optname, optval, optlen);
}
int
HAL_SOCK_getpeername(SOCK_SOCKET socket, struct SOCK_sockaddr* name, int* namelen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::GetPeerName(socket, name, namelen);
}
int
HAL_SOCK_getsockname(SOCK_SOCKET socket, struct SOCK_sockaddr* name, int* namelen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::GetSockName(socket, name, namelen);
}
int
HAL_SOCK_recvfrom(SOCK_SOCKET s, char* buf, int len, int flags, struct SOCK_sockaddr* from, int* fromlen)
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LWIP_SOCKETS_Driver::RecvFrom(s, buf, len, flags, from, fromlen);
}
int
HAL_SOCK_sendto(SOCK_SOCKET s, const char* buf, int len, int flags, const struct SOCK_sockaddr* to, int tolen)
{
    NATIVE_PROFILE_PAL_NETWORK();
...
HRESULT
HAL_SOCK_CONFIGURATION_LoadAdapterConfiguration(HAL_Configuration_NetworkInterface* config, uint32_t interfaceIndex)
{
    NATIVE_PROFILE_PAL_NETWORK();
...
void
HAL_SOCK_EventsSet(uint32_t events)
{
    NATIVE_PROFILE_PAL_NETWORK();
