    ...
    bool init)
{
    ...
    do
    {
        ...
        while (current)
        {
            if (0 == hal_stricmp(current->m_nameSpace, nameSpace))
            {
                nameOK = false;
                break;
            }
    ...
    memcpy(fsv->m_nameSpace, nameSpace, FS_NAME_MAXLENGTH);
...
FileSystemVolume *FileSystemVolumeList::FindVolume(LPCSTR nameSpace, uint32_t nameSpaceLength)
{
