...
__nfweak bool  CPU_SPI_Initialize(int bus) { (void)bus; return true; }
__nfweak void  CPU_SPI_Uninitialize(int bus) { (void)bus; }
__nfweak SPI_OP_STATUS CPU_SPI_Op_Status(uint32_t deviceHandle) { (void)deviceHandle; return SPI_OP_STATUS::SPI_OP_COMPLETE; }
...
__nfweak bool CPU_SPI_Remove_Device(uint32_t deviceHandle) { (void)deviceHandle;  return true; };
...
static bool getDevice(uint32_t handle, uint8_t& spiBus, int& deviceIndex)
{
	...
	if (type != CPU_DEVICE_TYPE_SPI ||
		spiBus >= CPU_SPI_PortsCount() ||
		deviceIndex >= NUM_SPI_BUSES
		)
		return false;
...
HRESULT nanoSPI_ReserveBusPins(int spiBus, bool reserve)
{
	...
	if (reserve)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				if (CPU_GPIO_PinIsBusy(pins[i])) return CLR_E_INVALID_PARAMETER;
			}
		}
	}
	...
	for (int i = 0; i < 3; i++)
	{
		if (CPU_GPIO_ReservePin(pins[i], reserve) == false) return CLR_E_INVALID_PARAMETER;
	}
...
HRESULT nanoSPI_OpenDeviceEx(SPI_DEVICE_CONFIGURATION& spiDeviceConfig, GPIO_PIN altMsk, GPIO_PIN altMiso, GPIO_PIN altMosi)
{
...
HRESULT nanoSPI_CloseDevice(uint32_t handle)
{
	...
	if (!getDevice(handle, spiBus, deviceIndex))  return CLR_E_INVALID_PARAMETER;
...
//
HRESULT nanoSPI_Write_Read(uint32_t handle, SPI_WRITE_READ_SETTINGS & swrs, uint8_t * writePtr, int32_t wsize, uint8_t * readPtr, int32_t readSize)
{
	uint8_t spiBus;
	int    deviceIndex;
	...
	return CPU_SPI_nWrite_nRead(spiconfig[spiBus].deviceHandles[deviceIndex], spiconfig[spiBus].deviceCongfig[deviceIndex], swrs, writePtr, wsize, readPtr, readSize);
}
