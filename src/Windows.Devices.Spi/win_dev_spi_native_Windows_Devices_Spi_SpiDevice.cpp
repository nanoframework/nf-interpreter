...
bool IsLongRunningOperation(uint32_t writeSize, uint32_t readSize, bool fullDuplex, bool bufferIs16bits, float byteTime, uint32_t& estimatedDurationMiliseconds)
{
    if (bufferIs16bits)
    {
...
HRESULT Library_win_dev_spi_native_Windows_Devices_Spi_SpiDevice::NativeTransfer( CLR_RT_StackFrame& stack, bool data16Bits)
{
    NANOCLR_HEADER();
    {
        ...
        if (stack.m_customState == 0)
        {
            ...
            if (data16Bits) databitLength = 16;
            ...
            bool dataTransfer16 = (pConfig[SpiConnectionSettings::FIELD___databitLength].NumericByRef().s4 == 16) && data16Bits;
            ...
            if (isLongRunningOperation)
            {
                ...
                if (writeBuffer != NULL) CLR_RT_ProtectFromGC gcWriteBuffer(*writeBuffer);
                if (readBuffer != NULL) CLR_RT_ProtectFromGC gcReadBuffer(*readBuffer);
            ...
            hr = nanoSPI_Write_Read(deviceId, rws, (uint8_t *)writeData, (int32_t)writeSize, (uint8_t * )readData, (int32_t)readSize);
...
{
    {
        NANOCLR_HEADER();
        {
            ...
            CLR_RT_HeapBlock* config = pThis[Library_win_dev_spi_native_Windows_Devices_Spi_SpiDevice::FIELD___connectionSettings].Dereference();
