...
/*
    ...
    if (CPU_GPIO_ReservePin(GPIOI_LCD_INT, true))
    {
        if (CPU_GPIO_EnableInputPin(STMPE811QTR_Interrupt, 0, touchIsrProc, ISR_Param, IntEdge, driveMode))
        {
            ...
            uint8_t regValue = 0;
            regValue =
                (STMPE811QTR_G_MODE_INTERRUPT_TRIGGER & (STMPE811QTR_G_MODE_INTERRUPT_MASK >> STMPE811QTR_G_MODE_INTERRUPT_SHIFT))
...
/*
    ...
    CLR_UINT8 regValue =
        (STMPE811QTR_G_MODE_INTERRUPT_POLLING & (STMPE811QTR_G_MODE_INTERRUPT_MASK >> STMPE811QTR_G_MODE_INTERRUPT_SHIFT))
        << STMPE811QTR_G_MODE_INTERRUPT_SHIFT;
    CLR_UINT8 values[2] = {STMPE811QTR_GMODE_REG, regValue};
    g_TouchInterface.Write_Read(values, 2, 0);
*/
