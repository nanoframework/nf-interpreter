...
enum loopingState { freeBlockSearch, found, failed };
enum blockStatus { heapStart = 0xFACEFACE, blockFree = 0xDEADBEEF, blockBusy = 0xDEADBE5E, heapEnd = 0xDEADDEAD };
...
void* GraphicsMemoryHeap::Allocate(CLR_UINT32 requestedSize)
{
    ...
    while (looping == freeBlockSearch)
    {
        ...
        {
            if (currentBlk->dataLength >= requestedSize + blockHeaderSize)      // Room for the requested data and the next block (to split the region)
            {
                ...
                struct BlockHeader* insertedBlockHeader = (struct BlockHeader*)((CLR_UINT32)currentBlk + blockHeaderSize + requestedSize);
                ...
                insertedBlockHeader->dataLength = (CLR_UINT32)insertedBlockHeader->next - (CLR_UINT32)insertedBlockHeader - blockHeaderSize;
    ...
    if (looping == found)
    {
        CLR_UINT32 ptrDataArea = ((CLR_UINT32)currentBlk + blockHeaderSize);  // return a pointer to start of allocated memory
...
bool GraphicsMemoryHeap::Release(void* pHeapBlockData)
{
    ...
    if (currentBlk >= ptrfirstBlockHeader && currentBlk < ptrlastBlockHeader)
    {
        ...
        // If the previous block is free, then subsume current block into previous including data length and block header
