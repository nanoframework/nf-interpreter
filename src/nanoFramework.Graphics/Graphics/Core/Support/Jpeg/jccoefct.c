...
METHODDEF(bool) compress_data JPP((j_compress_ptr cinfo, JSAMPIMAGE input_buf));
#ifdef FULL_COEF_BUFFER_SUPPORTED
METHODDEF(bool) compress_first_pass JPP((j_compress_ptr cinfo, JSAMPIMAGE input_buf));
METHODDEF(bool) compress_output JPP((j_compress_ptr cinfo, JSAMPIMAGE input_buf));
#endif
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
    {
        ...
        if (coef->iMCU_row_num == last_iMCU_row)
        {
            ...
            for (block_row = block_rows; block_row < compptr->v_samp_factor; block_row++)
            {
                ...
                jzero_far((void FAR *)thisblockrow, (CLR_INT32)(blocks_across * SIZEOF(JBLOCK)));
