...
typedef struct {
    ...
    CLR_INT32* Cr_g_tab;      /* => table for Cr to G conversion */
    CLR_INT32* Cb_g_tab;      /* => table for Cb to G conversion */
...
build_ycc_rgb_table(j_decompress_ptr cinfo)
{
    ...
    cconvert->Cb_b_tab = (int*)
        (*cinfo->mem->alloc_small) ((j_common_ptr)cinfo, JPOOL_IMAGE,
        (MAXJSAMPLE + 1) * SIZEOF(int));
    cconvert->Cr_g_tab = (CLR_INT32*)
        (*cinfo->mem->alloc_small) ((j_common_ptr)cinfo, JPOOL_IMAGE,
        (MAXJSAMPLE + 1) * SIZEOF(CLR_INT32));
    cconvert->Cb_g_tab = (CLR_INT32*)
        (*cinfo->mem->alloc_small) ((j_common_ptr)cinfo, JPOOL_IMAGE,
        (MAXJSAMPLE + 1) * SIZEOF(CLR_INT32));
    ...
    JSAMPARRAY output_buf, int num_rows)
{
    ...
    register CLR_INT32* Crgtab = cconvert->Cr_g_tab;
    register CLR_INT32* Cbgtab = cconvert->Cb_g_tab;
    SHIFT_TEMPS
    ...
    JSAMPARRAY output_buf, int num_rows)
{
    ...
    const CLR_INT32* Crgtab = cconvert->Cr_g_tab;
    const CLR_INT32* Cbgtab = cconvert->Cb_g_tab;
    SHIFT_TEMPS
    ...
    JSAMPARRAY output_buf, int num_rows)
{
    ...
    while (--num_rows >= 0) {
        ...
        for (col = 0; col < num_cols; col++) {
            ...
            *((CLR_UINT16*)outptr) = ((R >> 3) << 11) | ((G >> 2) << 5) | (B >> 3);
    ...
    JSAMPARRAY output_buf, int num_rows)
{
    ...
    while (--num_rows >= 0) {
        ...
        for (col = 0; col < num_cols; col++) {
            ...
            *((CLR_UINT16*)outptr) = ((value >> 3) << 11) | ((value >> 2) << 5) | (value >> 3);
