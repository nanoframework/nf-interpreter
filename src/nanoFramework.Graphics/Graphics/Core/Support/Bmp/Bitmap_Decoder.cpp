...
HRESULT BmpDecoder::BmpInitOutput(const CLR_UINT8* src, CLR_UINT32 srcSize) {
    ...
    NANOCLR_CHECK_HRESULT(source.Skip(sizeof(
        BITMAPFILEHEADER))); // Note that pbmfh->bfSize records the size of the
    ...
    case 8:
        if (pbmih->biClrUsed != 0) {
            ...
            paletteDepth = (CLR_UINT8)((pbmfh->bfOffBits - sizeof(BITMAPINFOHEADER) -
...
HRESULT BmpDecoder::BmpStartOutput(CLR_GFX_Bitmap* bitmap) {
    ...
    switch (encodingType) {
    case Bmp16Bit_565:
        param.srcWidthInBytes =
            ((width * 16 + 31) / 32) * 4; // each line is 4-byte aligned in size
        break;

    case Bmp32Bit_ARGB:
        param.srcWidthInBytes =
            ((width * 32 + 31) / 32) * 4; // each line is 4-byte aligned in size
        break;

    case Bmp24Bit_RGB:
        param.srcWidthInBytes =
            ((width * 24 + 31) / 32) * 4; // each line is 4-byte aligned in size
        break;

    case Bmp8Bit_Indexed:
        param.srcWidthInBytes =
            ((width * 8 + 31) / 32) * 4; // each line is 4-byte aligned in size
    ...
    config = PAL_GFX_Bitmap::c_SetPixelsConfig_NoClip |
        PAL_GFX_Bitmap::c_SetPixelsConfig_NoClipChecks;
...
#endif
    bitmap->SetPixelsHelper(rect, config, (GFX_SetPixelsCallback)&BmpOutputHelper,
        &param);
#if __GNUC__ > 6
