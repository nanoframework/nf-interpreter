...
HRESULT Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::EnableInterrupt___VOID(
    CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();

    CLR_RT_HeapBlock_NativeEventDispatcher *pNativeDisp = nullptr;
    ...
    if (pNativeDisp->driverMethods == nullptr)
    {
...
HRESULT Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::DisableInterrupt___VOID(
    CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
...
HRESULT Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::Dispose___VOID__BOOLEAN(
    CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
...
HRESULT Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::_ctor___VOID__STRING__U8(
    CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
    ...
    driverName = stack.Arg1().RecoverString();
    FAULT_ON_NULL(driverName);
    driverData = stack.Arg2().NumericByRef().u8;

    // Throw nullptr exception if string is empty.
    ...
    if (pDriverMethods->initProcessor == nullptr || pDriverMethods->enableProcessor == nullptr ||
        pDriverMethods->cleanupProcessor == nullptr)
    {
        NANOCLR_CHECK_HRESULT(CLR_E_DRIVER_NOT_REGISTERED);
    }
    ...
    NANOCLR_CHECK_HRESULT(
        CLR_RT_HeapBlock_NativeEventDispatcher::CreateInstance(*pThis, pThis[FIELD___NativeEventDispatcher]));
...
CLR_RT_ObjectToEvent_Source *Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::
    GetEventDispReference(CLR_RT_StackFrame &stack)
{
...
HRESULT Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::GetEventDispatcher(
    CLR_RT_StackFrame &stack,
...
CLR_RT_HeapBlock_NativeEventDispatcher *Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::
    GetEventDispatcher(CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_HARDWARE();

    CLR_RT_ObjectToEvent_Source *src = GetEventDispReference(stack);

    return (src == nullptr) ? nullptr : (CLR_RT_HeapBlock_NativeEventDispatcher *)src->m_eventPtr;
}
