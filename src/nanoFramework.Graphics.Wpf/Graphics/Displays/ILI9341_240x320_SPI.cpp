...
bool DisplayDriver::Initialize()
{
	...
	CLR_UINT8  Power_Control_B_Data[] = { 0x00, 0x83, 0X30 };
	CLR_UINT8  Power_On_Sequence_Data[] = { 0x64, 0x03, 0X12, 0X81 };
	CLR_UINT8  Driver_Timing_Control_A_Data[] = { 0x85, 0x01, 0x79 };

	CLR_UINT8  Power_Control_A_Data[] = { 0x39, 0x2C, 0x00, 0x34, 0x02 };
	CLR_UINT8  Pump_Ratio_Control_Data[] = { 0x20 };
	CLR_UINT8  Driver_Timing_Control_B_Data[] = { 0x00, 0x00 };
	CLR_UINT8  Power_Control_1_Data[] = { 0x26 };
	CLR_UINT8  Power_Control_2_Data[] = { 0x11 };

	CLR_UINT8  VCOM_Control_1_Data[] = { 0x35, 0x3E };
	CLR_UINT8  VCOM_Control_2_Data[] = { 0xBE };
	CLR_UINT8  Memory_Access_Control_Data[] = { 0x28 };				// Portrait?
	CLR_UINT8  Pixel_Format_Set_Data[] = { 0x55 };					// 0x55 -> 16 bit 
	CLR_UINT8  Frame_Rate_Control_Normal_Data[] = { 0x00, 0x1B };
	CLR_UINT8  Enable_3G_Data[] = { 0x08 };
	CLR_UINT8  Gamma_Set_Data[] = { 0x01 }; // Gamma curve selected (0x01, 0x02, 0x04, 0x08)
	CLR_UINT8  Positive_Gamma_Correction_Data[] = { 0x1F, 0x1A, 0x18, 0x0A, 0x0F, 0x06, 0x45, 0X87, 0x32, 0x0A, 0x07, 0x02, 0x07, 0x05, 0x00 }; //gamma set 4
	CLR_UINT8  Negative_Gamma_Correction_Data[] = { 0x00, 0x25, 0x27, 0x05, 0x10, 0x09, 0x3A, 0x78, 0x4D, 0x05, 0x18, 0x0D, 0x38, 0x3A, 0x1F };
	CLR_UINT8  Column_Address_Set_Data[] = { 0x00, 0x00, 0x00, 0xEF };  // Size = 239 - Note endian
	CLR_UINT8  Page_Address_Set_Data[] = { 0x00, 0x00, 0x01, 0x3f };  // Size = 319
	CLR_UINT8  Entry_Mode_Set_Data[] = { 0x07 };  // Entry mode set
	CLR_UINT8  Display_Function_Control_Data[] = { 0x0A, 0x82, 0x27, 0x00 };

	// g_DisplayInterface.SendCommand(SOFTWARE_RESET);
	// g_DisplayInterface.SendCommand(Display_OFF);

	g_DisplayInterface.SendCommandByteAndData(Power_Control_B, Power_Control_B_Data, 3);
	g_DisplayInterface.SendCommandByteAndData(Power_On_Sequence, Power_On_Sequence_Data, 4);
	g_DisplayInterface.SendCommandByteAndData(Driver_Timing_Control_A, Driver_Timing_Control_A_Data, 3);
	g_DisplayInterface.SendCommandByteAndData(Power_Control_A, Power_Control_A_Data, 5);
	g_DisplayInterface.SendCommandByteAndData(Pump_Ratio_Control, Pump_Ratio_Control_Data, 1);
	g_DisplayInterface.SendCommandByteAndData(Driver_Timing_Control_B, Driver_Timing_Control_B_Data, 2);
	g_DisplayInterface.SendCommandByteAndData(Power_Control_1, Power_Control_1_Data, 1);
	g_DisplayInterface.SendCommandByteAndData(Power_Control_2, Power_Control_2_Data, 1);
	g_DisplayInterface.SendCommandByteAndData(VCOM_Control_1, VCOM_Control_1_Data, 2);
	g_DisplayInterface.SendCommandByteAndData(VCOM_Control_2, VCOM_Control_2_Data, 1);
	g_DisplayInterface.SendCommandByteAndData(Memory_Access_Control, Memory_Access_Control_Data, 1);  // Portrait?
	g_DisplayInterface.SendCommandByteAndData(Pixel_Format_Set, Pixel_Format_Set_Data, 1);   // 0x55 -> 16 bit 
	g_DisplayInterface.SendCommandByteAndData(Frame_Rate_Control_Normal, Frame_Rate_Control_Normal_Data, 2);
	g_DisplayInterface.SendCommandByteAndData(Enable_3G, Enable_3G_Data, 1);
	g_DisplayInterface.SendCommandByteAndData(Gamma_Set, Gamma_Set_Data, 1); // Gamma curve selected (0x01, 0x02, 0x04, 0x08)
	g_DisplayInterface.SendCommandByteAndData(Positive_Gamma_Correction, Positive_Gamma_Correction_Data, 15); //gamma set 4
	g_DisplayInterface.SendCommandByteAndData(Negative_Gamma_Correction, Negative_Gamma_Correction_Data, 15);
	g_DisplayInterface.SendCommandByteAndData(Column_Address_Set, Column_Address_Set_Data, 4);  // Size = 239
	g_DisplayInterface.SendCommandByteAndData(Page_Address_Set, Page_Address_Set_Data, 4);  // Size = 319
	g_DisplayInterface.SendCommandByte(Memory_Write);
	g_DisplayInterface.SendCommandByteAndData(Entry_Mode_Set, Entry_Mode_Set_Data, 1);  // Entry mode set
	g_DisplayInterface.SendCommandByteAndData(Display_Function_Control, Display_Function_Control_Data, 4);

	g_DisplayInterface.SendCommandByte(Sleep_Out);
	g_DisplayInterface.DisplayCommandDelay(20);																 // Send Sleep Out command to display : no parameter
	g_DisplayInterface.SendCommandByte(Display_ON);
	g_DisplayInterface.DisplayCommandDelay(20);																 // Send Sleep Out command to display : no parameter
	g_DisplayInterface.SendCommandByte(NOP);   // End of sequence
	g_DisplayInterface.DisplayCommandDelay(20);																 // Send Sleep Out command to display : no parameter
...
bool DisplayDriver::ChangeOrientation(DisplayOrientation newOrientation)
{
	...
	CLR_UINT8  Memory_Access_Control_Data_Portrait[] = { 0x48 };
	CLR_UINT8  Memory_Access_Control_Data_Portrait180[] = { 0x88 };
	CLR_UINT8  Memory_Access_Control_Data_Landscape[] = { 0xE8 };
	CLR_UINT8  Memory_Access_Control_Data_Landscape180[] = { 0x28 };
	...
	case DisplayOrientation::PORTRAIT:
		...
		g_DisplayInterface.SendCommandByteAndData(Memory_Access_Control, Memory_Access_Control_Data_Portrait, 1);
		break;
	case DisplayOrientation::PORTRAIT180:
		Attributes.Height = Attributes.LongerSide;
		Attributes.Width = Attributes.ShorterSide;
		g_DisplayInterface.SendCommandByteAndData(Memory_Access_Control, Memory_Access_Control_Data_Portrait180, 1); /* X and Y axes non-inverted */
		break;
	case DisplayOrientation::LANDSCAPE:
		Attributes.Height = Attributes.ShorterSide;
		Attributes.Width = Attributes.LongerSide;
		g_DisplayInterface.SendCommandByteAndData(Memory_Access_Control, Memory_Access_Control_Data_Landscape, 1); /* Invert X and Y axes */
		break;
	case DisplayOrientation::LANDSCAPE180:
		Attributes.Height = Attributes.ShorterSide;
		Attributes.Width = Attributes.LongerSide;
		g_DisplayInterface.SendCommandByteAndData(Memory_Access_Control, Memory_Access_Control_Data_Landscape180, 1); /* Invert X and Y axes */
		break;
	}
...
{
	CLR_UINT8 POWER_STATE_Data_ON[] = { 0x0000 };
	CLR_UINT8 POWER_STATE_Data_SLEEP[] = { 0x0001 };
	switch (powerState)
	{
	default:
		// illegal fall through to Power on
	case PowerSaveState::NORMAL:
		g_DisplayInterface.SendCommandByteAndData(POWER_STATE, POWER_STATE_Data_ON, 1);  // leave sleep mode
		break;
	case PowerSaveState::SLEEP:
		g_DisplayInterface.SendCommandByteAndData(POWER_STATE, POWER_STATE_Data_SLEEP, 1);  // enter sleep mode
		break;
	}
	return;
}
...
void DisplayDriver::DisplayBrightness(CLR_INT16 brightness)
{
	...
	CLR_UINT8  Write_Display_Brightness_Data[] = { (CLR_UINT8)brightness };
	g_DisplayInterface.SendCommandByteAndData(Write_Display_Brightness, Write_Display_Brightness_Data, 1);
	return;
...
void DisplayDriver::BitBltEx(int x, int y, int width, int height, CLR_UINT32 data[])
{
	...
	// Will need to code for orientation, will change the way the data is presented, maybe, or prepare it 
	// the proper way with orientation changed before hand?????? Note: by Terry
	...
	g_DisplayInterface.SendCommandByteAndData(Column_Address_Set, Column_Address_Set_Data, 4);
	...
	for (int iSpiTransfer = 0; iSpiTransfer < TotalNumberOfSpiTransfers; iSpiTransfer++) {
		...
		g_DisplayInterface.SendCommandByteAndData(Page_Address_Set, Page_Address_Set_Data, 4);

		// Send the data to the ILI9341 via SPI
		g_DisplayInterface.SendCommandByteAndData(Memory_Write, Attributes.TransferBuffer, numberOfBytesPerTransfer);
