...
HRESULT Library_nf_system_resourcemanager_System_Resources_ResourceManager::GetObjectInternal___OBJECT__I2(
    CLR_RT_StackFrame &stack)
{
    ...
    NANOCLR_CHECK_HRESULT(Library_corlib_native_System_Reflection_Assembly::GetTypeDescriptor(
        *pThis[FIELD___assembly].Dereference(),
        assm));
        ...
        switch (resource->kind)
        {
            ...
            case CLR_RECORD_RESOURCE::RESOURCE_Bitmap:
            {
                ...
                NANOCLR_SET_AND_LEAVE(
                    CLR_GFX_Bitmap::CreateInstance(ptr[CLR_GFX_Bitmap::FIELD__m_bitmap], buf, size, pAssm));
            }
            break;
...
HRESULT Library_nf_system_resourcemanager_System_Resources_ResourceManager::GetObjectInternal___OBJECT__I2__I4__I4(
    CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
...
HRESULT Library_nf_system_resourcemanager_System_Resources_ResourceManager::
    FindResource___STATIC__I4__STRING__SystemReflectionAssembly(CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
    ...
    const char *szText = pArgs[0].RecoverString();
    FAULT_ON_NULL(szText);

    NANOCLR_CHECK_HRESULT(
        Library_corlib_native_System_Reflection_Assembly::GetTypeDescriptor(*pArgs[1].Dereference(), assm));
...
HRESULT Library_nf_system_resourcemanager_System_Resources_ResourceManager::
    GetObject___STATIC__OBJECT__SystemResourcesResourceManager__SystemEnum(CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
    ...
    if (stack.m_customState == 0)
    {
        ...
        if (blkEnum->DataType() != DATATYPE_I2 && blkEnum->DataType() != DATATYPE_U2)
...
HRESULT Library_nf_system_resourcemanager_System_Resources_ResourceManager::
    GetObject___STATIC__OBJECT__SystemResourcesResourceManager__SystemEnum__I4__I4(CLR_RT_StackFrame &stack)
{
    NATIVE_PROFILE_CLR_CORE();
    NANOCLR_HEADER();
