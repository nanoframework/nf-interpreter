...
HRESULT Library_corlib_native_System_DateTime::_ctor___VOID__I4__I4__I4__I4__I4__I4__I4(CLR_RT_StackFrame &stack)
{
    ...
    /// Our current supported range is between 1601 and 3000. Years before 1582 requires different calculation (see
    /// explanation in time_decl.h), same way years after 3000 will not hold the simple arithmetic which we are using.
    /// More complex calculations outside these range are not worth the CPU cycle and codesize.
    // some checks below have to use the parameter because the st struct uses unsigned types
    if ((st.wYear < 1601) || (st.wYear > 3000) || (st.wMonth < 1) || (st.wMonth > 12) || (st.wDay < 1) ||
        (st.wDay > 31) || (pArg[3].NumericByRef().s4 < 0) || (st.wHour > 23) || (pArg[4].NumericByRef().s4 < 0) ||
        (st.wMinute > 59) || (pArg[5].NumericByRef().s4 < 0) || (st.wSecond > 59) ||
        (st.wMilliseconds < pArg[6].NumericByRef().s4) || (st.wMilliseconds > 1000))
    {
...
HRESULT Library_corlib_native_System_DateTime::GetDateTimePart___I4__SystemDateTimeDateTimePart(
...
CLR_INT64 *Library_corlib_native_System_DateTime::NewObject(CLR_RT_HeapBlock &ref)
{
    ...
    dtType.InitializeFromType(g_CLR_RT_WellKnownTypes.DateTime);
