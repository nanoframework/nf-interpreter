...
HRESULT Library_corlib_native_System_Reflection_RuntimeFieldInfo::get_DeclaringType___SystemType(
    CLR_RT_StackFrame &stack)
{
    ...
    if (cls.InitializeFromField(fd) == false)
...
HRESULT Library_corlib_native_System_Reflection_RuntimeFieldInfo::GetValue___OBJECT__OBJECT(CLR_RT_StackFrame &stack)
{
...

    {
        ...
        if (argObj->IsTransparentProxy())
        {
            ...
            NANOCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.GetCurrentAppDomain()
                                      ->MarshalObject(*obj, dst, argObj->TransparentProxyAppDomain()));
        }
#endif
...
bool Library_corlib_native_System_Reflection_RuntimeFieldInfo::GetFieldDescriptor(
    CLR_RT_StackFrame &stack,
