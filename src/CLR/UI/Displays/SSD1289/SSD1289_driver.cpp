...
#endif
void Cpu_Delay100US(CLR_UINT16 us100)
{
	CLR_UINT16 i;
    CLR_UINT16 x = 0;
	while((us100--))
...
void Delay_ms_Common(CLR_UINT16 period)   //delay routine (milliseconds)
{
    while (period != 0) 
	{      
      Cpu_Delay100US(10);
...
void SSD1289_Driver::SendCmdWord(CLR_UINT16 cmd)
{
...
void SSD1289_Driver::SendDataWord(CLR_UINT16 value)
{
	// wait write buffer not full flag        
	while (!(LCD_SPI_SR & SPI_SR_TFFF_MASK)){}; 
	// Assert CS0, Use config 0
	LCD_SPI_PUSHR = SPI_PUSHR_PCS(1 << (LCD_SPI_PCS_ID)) | SPI_PUSHR_CTAS(0) | SPI_PUSHR_TXDATA((CLR_UINT16)value);

	while (!(LCD_SPI_SR & SPI_SR_TCF_MASK)){};// while shift-out complete
	LCD_SPI_SR = SPI_SR_TCF_MASK;           // clear flag
	return;
}
void SSD1289_Driver::SendCommandData(const CLR_UINT16 data[], CLR_UINT16 count)
{
  int i;   
  for (i=0; i<count; i+=2)
  {
...
bool SSD1289_Driver::SetWindow(CLR_UINT16 x1, CLR_UINT16 y1, CLR_UINT16 x2, CLR_UINT16 y2)
{
	CLR_UINT16 x1_x2;
...
void SSD1289_Driver::BitBltEx( int x, int y, int width, int height, CLR_UINT32 data[] )
{
    NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
    ...
    CLR_UINT16 offset = (y * g_SSD1289_Config.Width) + x;
...
void SSD1289_Driver::BitBlt( int width, int height, int widthInWords, CLR_UINT32 data[], bool fUseDelta )
{
     NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
...
CLR_UINT32 SSD1289_Driver::PixelsPerWord()
{
    NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
    return ((8*sizeof(CLR_UINT32)) / g_SSD1289_Config.BitsPerPixel);
} //done
CLR_UINT32 SSD1289_Driver::TextRows()
{
    NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
    return (g_SSD1289_Config.Height / Font_Height());
} //done
CLR_UINT32 SSD1289_Driver::TextColumns()
{
    NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
    return (g_SSD1289_Config.Width / Font_Width());
} //done
CLR_UINT32 SSD1289_Driver::WidthInWords()
{
     NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
     return ((g_SSD1289_Config.Width + (PixelsPerWord() - 1)) / PixelsPerWord());
} //done
CLR_UINT32 SSD1289_Driver::SizeInWords()
{
     NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
     return (WidthInWords() * g_SSD1289_Config.Height);
} //done
CLR_UINT32 SSD1289_Driver::SizeInBytes()
{
     NATIVE_PROFILE_HAL_DRIVERS_DISPLAY();
