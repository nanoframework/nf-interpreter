...
typedef struct {
   ...
   */
  CLR_UINT8 h_expand[MAX_COMPONENTS];
  CLR_UINT8 v_expand[MAX_COMPONENTS];
} my_upsampler;
...
#else
  register CLR_INT32 thiscolsum, lastcolsum, nextcolsum;
#endif
  register JDIMENSION colctr;
  int inrow, outrow, v;
...
jinit_upsampler (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
	...
	upsample->methods[ci] = h2v2_upsample;
      ...
      upsample->h_expand[ci] = (CLR_UINT8) (h_out_group / h_in_group);
      upsample->v_expand[ci] = (CLR_UINT8) (v_out_group / v_in_group);
    } else
