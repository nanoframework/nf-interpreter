...
HRESULT CLR_RT_HeapBlock_Finalizer::CreateInstance(CLR_RT_HeapBlock *object, const CLR_RT_TypeDef_Instance &inst)
{
    ...
    do
    {
        ...
        {
            int pos = inst2.m_target->FirstMethod;
            int end = pos + inst2.m_target->VirtualMethodCount + inst2.m_target->InstanceMethodCount;
            ...
            if (pos < end)
            {
                CLR_RT_HeapBlock_Finalizer *fin = EVENTCACHE_EXTRACT_NODE_NOALLOC(
                    g_CLR_RT_EventCache,
                    CLR_RT_HeapBlock_Finalizer,
                    DATATYPE_FINALIZER_HEAD);
                CHECK_ALLOCATION(fin);
