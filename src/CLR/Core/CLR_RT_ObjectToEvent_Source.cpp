...
HRESULT CLR_RT_ObjectToEvent_Source::CreateInstance(
    ...
    CLR_RT_HeapBlock &reference)
{
    ...
    CLR_RT_ObjectToEvent_Source *oe =
        EVENTCACHE_EXTRACT_NODE(g_CLR_RT_EventCache, CLR_RT_ObjectToEvent_Source, DATATYPE_OBJECT_TO_EVENT);
    CHECK_ALLOCATION(oe);
...
{
    NATIVE_PROFILE_CLR_CORE();
    if (m_objectPtr && m_objectPtr->IsAlive())
        return;
