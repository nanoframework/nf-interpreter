...
HRESULT CLR_RT_HeapBlock_BinaryBlob::CreateInstance(
    ...
    CLR_UINT32 flags)
{
    ...
    CLR_RT_HeapBlock_BinaryBlob *obj = Allocate(length, flags);
...
CLR_RT_HeapBlock_BinaryBlob *CLR_RT_HeapBlock_BinaryBlob::Allocate(CLR_UINT32 length, CLR_UINT32 flags)
{
    ...
    {
        obj = EVENTCACHE_EXTRACT_NODE_AS_BYTES(
            g_CLR_RT_EventCache,
            CLR_RT_HeapBlock_BinaryBlob,
    ...
    {
        obj = (CLR_RT_HeapBlock_BinaryBlob *)
                  g_CLR_RT_ExecutionEngine.ExtractHeapBytesForObjects(DATATYPE_BINARY_BLOB_HEAD, flags, totLength);
    }
