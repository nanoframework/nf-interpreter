...
HRESULT CLR_RT_HeapBlock_Timer::CreateInstance(CLR_UINT32 flags, CLR_RT_HeapBlock &owner, CLR_RT_HeapBlock &tmRef)
{
    ...
    //
    timer = EVENTCACHE_EXTRACT_NODE(g_CLR_RT_EventCache, CLR_RT_HeapBlock_Timer, DATATYPE_TIMER_HEAD);
    CHECK_ALLOCATION(timer);
...
{
    NATIVE_PROFILE_CLR_CORE();
    if ((m_flags & CLR_RT_HeapBlock_Timer::c_Recurring) && (m_flags & CLR_RT_HeapBlock_Timer::c_EnabledTimer))
    {
...
void CLR_RT_HeapBlock_Timer::AdjustNextFixedExpire(const SYSTEMTIME &systemTime, bool fNext)
{
    ...
    {
        case CLR_RT_HeapBlock_Timer::c_SecondChange:
            add = TIME_CONVERSION__ONESECOND * (CLR_INT64)TIME_CONVERSION__TO_SECONDS;
            break;
        case CLR_RT_HeapBlock_Timer::c_MinuteChange:
            add = TIME_CONVERSION__ONEMINUTE * (CLR_INT64)TIME_CONVERSION__TO_SECONDS;
            st.wSecond = 0;
            break;
        case CLR_RT_HeapBlock_Timer::c_HourChange:
            add = TIME_CONVERSION__ONEHOUR * (CLR_INT64)TIME_CONVERSION__TO_SECONDS;
            st.wSecond = 0;
            st.wMinute = 0;
            break;
        case CLR_RT_HeapBlock_Timer::c_DayChange:
            add = TIME_CONVERSION__ONEDAY * (CLR_INT64)TIME_CONVERSION__TO_SECONDS;
            st.wSecond = 0;
            st.wMinute = 0;
            st.wHour = 0;
            break;
...
bool CLR_RT_HeapBlock_Timer::CheckDisposed(CLR_RT_StackFrame &stack)
{
    ...
    if (!FAILED(CLR_RT_HeapBlock_Timer::ExtractInstance(
            pThis[Library_corlib_native_System_Threading_Timer::FIELD___timer],
            timer)))
    {
...
HRESULT CLR_RT_HeapBlock_Timer::ConfigureObject(CLR_RT_StackFrame &stack, CLR_UINT32 flags)
{
    ...
    NANOCLR_CHECK_HRESULT(CLR_RT_HeapBlock_Timer::ExtractInstance(
        pThis[Library_corlib_native_System_Threading_Timer::FIELD___timer],
        timer));
    ...
    if (flags & CLR_RT_HeapBlock_Timer::c_ACTION_Change)
    {
        ...
        else if (flags & CLR_RT_HeapBlock_Timer::c_INPUT_TimeSpan)
        {
            ...
            pVal = Library_corlib_native_System_TimeSpan::GetValuePtr(args[0]);
            FAULT_ON_NULL(pVal);
            if (*pVal == -c_TickPerMillisecond)
                timer->m_timeExpire = TIMEOUT_INFINITE;
            else
                timer->m_timeExpire = *pVal;

            pVal = Library_corlib_native_System_TimeSpan::GetValuePtr(args[1]);
            FAULT_ON_NULL(pVal);
            if (*pVal == -c_TickPerMillisecond)
                timer->m_timeFrequency = TIMEOUT_INFINITE;
            else
                timer->m_timeFrequency = *pVal;
        }
        ...
        {
            if (flags & CLR_RT_HeapBlock_Timer::c_INPUT_Int32)
                timer->m_timeExpire *= TIME_CONVERSION__TO_MILLISECONDS;
        ...
        {
            if (flags & CLR_RT_HeapBlock_Timer::c_INPUT_Int32)
                timer->m_timeFrequency *= TIME_CONVERSION__TO_MILLISECONDS;
