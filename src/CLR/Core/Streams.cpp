...
HRESULT CLR_RT_HeapBlock_MemoryStream::CreateInstance(CLR_RT_HeapBlock_MemoryStream *&stream, CLR_UINT8 *buf, int len)
{
    ...
    stream = EVENTCACHE_EXTRACT_NODE(g_CLR_RT_EventCache, CLR_RT_HeapBlock_MemoryStream, DATATYPE_MEMORY_STREAM_HEAD);
    CHECK_ALLOCATION(stream);
...
HRESULT CLR_RT_HeapBlock_MemoryStream::Initialize(CLR_UINT8 *buf, CLR_UINT32 len)
{
    ...
    m_buffers.DblLinkedList_Initialize(); // CLR_RT_DblLinkedList m_buffers;           // EVENT HEAP - NO RELOCATION -
                                          // list of CLR_RT_HeapBlock_MemoryStream::Buffer
...
HRESULT CLR_RT_HeapBlock_MemoryStream::ToArray(CLR_RT_HeapBlock &ref)
{
    ...
    NANOCLR_FOREACH_NODE(Buffer, node, m_buffers)
    {
        memcpy(buf, node->m_payload, node->m_length);
        buf += node->m_length;
    }
    NANOCLR_FOREACH_NODE_END();
...
HRESULT CLR_RT_HeapBlock_MemoryStream::ReadBits(CLR_UINT64 &res, CLR_UINT32 bits)
{
    ...
    if (bits > 32)
    {
        NANOCLR_CHECK_HRESULT(ReadBits(ptr[1], bits - 32));
        bits = 32;
    }
    else
    {
...
HRESULT CLR_RT_HeapBlock_MemoryStream::ReadBits(CLR_UINT32 &res, CLR_UINT32 bits)
{
    ...
    while (bits > 0)
    {
        ...
        pos -= insert;
        val |= (((CLR_UINT32)m_current->m_data[m_pos] >> (m_avail - insert)) & mask) << pos;
        bits -= insert;
        m_avail -= insert;
    }
...
HRESULT CLR_RT_HeapBlock_MemoryStream::WriteBits(CLR_UINT64 res, CLR_UINT32 bits)
{
    ...
    if (bits > 32)
    {
        NANOCLR_CHECK_HRESULT(WriteBits((CLR_UINT32)(res >> 32), bits - 32));
        bits = 32;
    }
...
HRESULT CLR_RT_HeapBlock_MemoryStream::WriteBits(CLR_UINT32 val, CLR_UINT32 bits)
{
    ...
    while (bits > 0)
    {
        ...
        pos -= insert;
        m_current->m_payload[m_pos] |= (CLR_UINT8)(((val >> pos) & mask) << (m_avail - insert));
        bits -= insert;
        m_avail -= insert;
    }
