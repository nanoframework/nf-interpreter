...
//        - returns CLR_E_THREAD_WAITING - if the target object is not signaled - indicating that the caller thread is
//        waiting
//                                         for the object to be signaled
//
HRESULT CLR_RT_HeapBlock_WaitForObject::CreateInstance(
    ...
    bool fWaitAll)
{
    ...
    CLR_RT_HeapBlock_WaitForObject *wait = EVENTCACHE_EXTRACT_NODE_AS_BYTES(
        ...
        0,
        totLength);
    CHECK_ALLOCATION(wait);

    wait->m_timeExpire = timeExpire;
...
void CLR_RT_HeapBlock_WaitForObject::TryWaitForSignal(CLR_RT_Thread *th)
{
    ...
    if (wait)
    {
        if (CLR_RT_HeapBlock_WaitForObject::TryWaitForSignal(
                th,
                wait->GetWaitForObjects(),
                wait->m_cObjects,
                wait->m_fWaitAll))
        {
            th->Restart(true);
...
bool CLR_RT_HeapBlock_WaitForObject::TryWaitForSignal(
    CLR_RT_Thread *caller,
    CLR_RT_HeapBlock *objects,
    CLR_UINT32 cObjects,
    bool fWaitAll)
...
HRESULT CLR_RT_HeapBlock_WaitForObject::WaitForSignal(
    ...
    bool fWaitAll)
{
    ...
    if (stack.m_customState == 0)
    {
        ...
        {
            NANOCLR_CHECK_HRESULT(CLR_RT_HeapBlock_WaitForObject::CreateInstance(
