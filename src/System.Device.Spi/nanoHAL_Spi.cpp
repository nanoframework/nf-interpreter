...
__nfweak bool CPU_SPI_Initialize(uint8_t bus, SPI_DEVICE_CONFIGURATION &spiDeviceConfig)
{
    (void)bus;
    ...
    GPIO_PIN altMosi)
{
    ...
    if (hr != S_OK)
    {
        ...
        // Unreserve CS pin
        CPU_GPIO_ReservePin(spiconfig[spiDeviceConfig.Spi_Bus].deviceCongfig->DeviceChipSelect, false);
