...
HRESULT  nanoSPI_OpenDeviceEx(SPI_DEVICE_CONFIGURATION& Configuration, GPIO_PIN altMsk, GPIO_PIN altMiso, GPIO_PIN altMosi);
...
HRESULT  nanoSPI_Write_Read(uint32_t handle, SPI_WRITE_READ_SETTINGS & settings, uint8_t* writeData, int32_t writeSize, uint8_t* readData, int32_t readSize);
...
HRESULT  nanoSPI_start_nWrite_nRead(uint32_t handle, SPI_WRITE_READ_SETTINGS & settings, uint8_t* writeData, int32_t writeSize, uint8_t* readData, int32_t readSize);
